...
export class UsersController {
  ...
  @ApiQuery({ name: 'page', required: false, example: 1, minimum: 1 })
  @ApiQuery({
    name: 'limit',
    required: false,
    example: 5,
  ...
  findOne(@Param('id', UuidPipe) id: string, @Req() req: Request) {
    const authUser = req['user'] as { sub: string; admin: boolean };
    if (!authUser.admin && authUser.sub !== id) {
      throw new ForbiddenException('No puede consultar otro usuario');
    }
  ...
  update(
    ...
    const authUser = req['user'] as { sub: string; admin: boolean };
    if (!authUser.admin && authUser.sub !== id) {
      throw new ForbiddenException('No puede modificar otro usuario');
    }
    if (Object.keys(dto).length === 0) {
      throw new BadRequestException('Debe proporcionar al menos un campo');
    }
